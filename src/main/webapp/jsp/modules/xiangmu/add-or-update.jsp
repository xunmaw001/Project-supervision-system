                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="jiaoshiId" name="jiaoshiId" type="hidden">
                                    <div class="form-group col-md-6 xiangmuUuidNumberDiv">
                                        <label>项目编号</label>
                                        <input style="width: 450px" id="xiangmuUuidNumber" name="xiangmuUuidNumber" class="form-control"
                                               placeholder="项目编号">
                                    </div>
                                    <div class="form-group col-md-6 xiangmuNameDiv">
                                        <label>项目名称</label>
                                        <input style="width: 450px" id="xiangmuName" name="xiangmuName" class="form-control"
                                               placeholder="项目名称">
                                    </div>
                                    <div class="form-group col-md-6 xiangmuTypesDiv">
                                        <label>项目类型</label>
                                        <select style="width: 450px" id="xiangmuTypesSelect" name="xiangmuTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6 xiangmuJingfeiDiv">
                                        <label>所需经费</label>
                                        <input style="width: 450px" id="xiangmuJingfei" name="xiangmuJingfei" class="form-control"
                                               onchange="xiangmuJingfeiChickValue(this)" placeholder="所需经费">
                                    </div>
                                    <div class="form-group  col-md-6 xiangmuContentDiv">
                                        <label>项目详情</label>
...
<script>
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationXiangmuyesnotypesSelect(){
            var xiangmuYesnoTypesSelect = document.getElementById('xiangmuYesnoTypesSelect');
            if(xiangmuYesnoTypesSelect != null && xiangmuYesnoTypesOptions != null  && xiangmuYesnoTypesOptions.length > 0 ){
                for (var i = 0; i < xiangmuYesnoTypesOptions.length; i++) {
                    xiangmuYesnoTypesSelect.add(new Option(xiangmuYesnoTypesOptions[i].indexName,xiangmuYesnoTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(xiangmuYesnoTypesSelect != null && xiangmuYesnoTypesOptions != null  && xiangmuYesnoTypesOptions.length > 0 ) {
            for (var i = 0; i < xiangmuYesnoTypesOptions.length; i++) {
                if (xiangmuYesnoTypesOptions[i].codeIndex == ruleForm.xiangmuYesnoTypes) {//下拉框value对比,如果一致就赋值汉字
                        xiangmuYesnoTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function getContent() {
        ...
        if($("#xiangmuContentupload").length>0) {
            ...
            if (xiangmuContentEditor.hasContents()) {
                $('#xiangmuContent-input').attr('value', xiangmuContentEditor.getContent());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function xiangmuJingfeiChickValue(e){
        ...
        function xiangmuDeleteChickValue(e){
            var this_val = e.value || 0;
            var reg=/^[1-9][0-9]*$/;
            if(!reg.test(this_val)){
                e.value = "";
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                xueshengId: "required",
                jiaoshiId: "required",
                xiangmuUuidNumber: "required",
                xiangmuName: "required",
                xiangmuTypes: "required",
                xiangmuJingfei: "required",
                xiangmuContent: "required",
                xiangmuYesnoTypes: "required",
                xiangmuYesnoText: "required",
            },
            messages: {
                xueshengId: "学生不能为空",
                jiaoshiId: "老师不能为空",
                xiangmuUuidNumber: "项目编号不能为空",
                xiangmuName: "项目名称不能为空",
                xiangmuTypes: "项目类型不能为空",
                xiangmuJingfei: "所需经费不能为空",
                xiangmuContent: "项目详情不能为空",
                xiangmuYesnoTypes: "审核状态不能为空",
                xiangmuYesnoText: "审核意见不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addxiangmu != null && addxiangmu != "" && addxiangmu != "null") {
            //注册表单验证
            $(validform());
            $("#xiangmuUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
                $(".xiangmuYesnoTypesDiv").remove();
                $(".xiangmuYesnoTextDiv").remove();
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            ...
            initializationXiangmuyesnotypesSelect();
            <!--  初始化级联表的下拉框  -->
